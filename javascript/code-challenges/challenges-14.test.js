...
const toTitleCase = (arr) => {
  let newArray = arr.map(element => element[0].toUpperCase() + element.substring(1));
  return(newArray)
  // Solution code here...
};
...
let biggerThanLuke = (arr) => {
  let finalArray = []
  arr.forEach(element => (parseInt(element.mass)))
  let luke = arr.find(element => element.name === "Luke Skywalker")
  arr.forEach(element => {
    if (element.mass > luke.mass) {
      console.log(element.mass, luke.mass)
