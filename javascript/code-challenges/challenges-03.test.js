...
const containsAnd = (arr) => {
  let contAnd = arr.filter(element => element.includes("and"));
  return(contAnd);
  // Solution code here...
};
...
const oddValues = (arr) => {
  let oddNums = arr.filter(element => (element % 2 != 0));
  return(oddNums);
  // Solution code here...
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let diffNums = arr.filter(element => !(forbiddenValues.includes(element)));
  return(diffNums);
  // Solution code here...
};
